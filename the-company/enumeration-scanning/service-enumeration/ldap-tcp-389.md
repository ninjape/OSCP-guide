...
```
kali@kali:~/tools$ ldapsearch -x -H ldap://10.129.95.180 -b 'DC=EGOTISTICAL-BANK,DC=LOCAL' 
# extended LDIF
#
# LDAPv3
...
replUpToDateVector:: AgAAAAAAAAAHAAAAAAAAAJqTZgKeNkBJlc4LFr+H0BYXkAEAAAAAAFwnn
 hgDAAAARsb/VEiFdUq/CcLUBWrijxaAAQAAAAAAHHgPFwMAAACrjO940UmFRLLC7Zxl/q+tDOAAAA
...
creationTime: 132979198375448635
...
```
kali@kali:~/htb$ ldapsearch -h 10.129.147.51 -x -s base namingcontexts
# extended LDIF
#
# LDAPv3
# base <> (default) with scope baseObject
...
```
kali@kali:~/htb$ ldapsearch -h 10.129.147.51 -x -b "DC=cascade,DC=local" > ldap-anonymous
```

## Dump only people with a query

```
ldapsearch -h 10.129.147.51 -x -b "DC=cascade,DC=local" '(objectClass=person)' > ldap-people

```

Decode base64 password found in ldap

```
kali@kali:~/htb$ echo clk0bjVldmE= | base64 -d
rY4n5eva
```
